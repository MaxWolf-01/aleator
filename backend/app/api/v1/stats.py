...
async def get_stats(session: AsyncSession = Depends(get_db_session)):
    """Get basic statistics about the Aleator service"""
    ...
    result = await session.exec(
        select(func.count(func.distinct(Decision.user_id)))
        .join(Roll)
        .where(Roll.created_at >= today_start)
    )
    active_users_today = result.one()
    
    # Rolls today
    result = await session.exec(
        select(func.count(Roll.id)).where(Roll.created_at >= today_start)
    )
    rolls_today = result.one()
    
    # Decisions created today
    result = await session.exec(
        select(func.count(Decision.id)).where(Decision.created_at >= today_start)
    )
