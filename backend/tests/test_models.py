...
def test_decision_relationships(session):
    ...
    decision = Decision(user_id=user.id, title="Have dessert?", type=DecisionType.BINARY)
...
def test_multi_choice_decision_relationships(session):
    ...
    decision = Decision(user_id=user.id, title="What to eat?", type=DecisionType.MULTI_CHOICE)
    session.add(decision)
    session.commit()
    session.refresh(decision)
